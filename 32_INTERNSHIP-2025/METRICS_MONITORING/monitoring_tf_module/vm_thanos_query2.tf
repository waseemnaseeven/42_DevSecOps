resource "outscale_vm" "vm_thanos_query2" {
    ...
    tags {
        key   = "name"
        value = "thanos query2 + node_exporter"
    }
}

output "private_ip_thanos_query2" {
    description = "private_ip of my vm_thanos_query2"
    value = outscale_vm.vm_thanos_query2.private_ip
}