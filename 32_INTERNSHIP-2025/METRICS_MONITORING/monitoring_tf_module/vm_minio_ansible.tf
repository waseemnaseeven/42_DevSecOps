resource "outscale_vm" "vm_minio_ansible" {
    ...
    tags {
        key   = "name"
        value = "ansible_vm_minio"
    }
}

output "private_ip_minio_ansible" {
    description = "private_ip of my vm_minio_ansible"
    value = outscale_vm.vm_minio_ansible.private_ip
}
