resource "outscale_vm" "vm_node_exporter_ansible" {
    ...
    tags {
        key   = "name"
        value = "ansible_vm_node_exporter"
    }
}
output "private_ip_node_exporter_ansible" {
    description = "private_ip of node_exporter_ansible"
    value = outscale_vm.vm_node_exporter_ansible.private_ip
}