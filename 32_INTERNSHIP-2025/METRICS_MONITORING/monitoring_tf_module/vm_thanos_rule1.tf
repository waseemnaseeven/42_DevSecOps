resource "outscale_vm" "vm_thanos_rule" {
    ...
    tags {
        key   = "name"
        value = "thanos rule + node_exporter"
    }
}

output "private_ip_thanos_rule" {
    description = "private_ip of my vm_thanos_rule"
    value = outscale_vm.vm_thanos_rule.private_ip
}