resource "outscale_vm" "vm_grafana2" {
    ...
    tags {
        key   = "name"
        value = "vm_grafana2 + node_exporter"
    }
}

output "private_ip_grafana2" {
    description = "private_ip of my vm_grafana2"
    value = outscale_vm.vm_grafana2.private_ip
}
