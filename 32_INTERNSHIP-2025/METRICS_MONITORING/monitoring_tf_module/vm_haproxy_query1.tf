resource "outscale_vm" "vm_haproxy_query1" {
    ...
    tags {
        key   = "name"
        value = "vm_haproxy_query1"
    }
}

output "private_ip_haproxy_query1" {
    description = "private_ip of my haproxy_query1"
    value = outscale_vm.vm_haproxy_query1.private_ip
}