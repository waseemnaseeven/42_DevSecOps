resource "outscale_vm" "vm_thanos_query3" {
    ...
    tags {
        key   = "name"
        value = "thanos query3 + node_exporter"
    }
}

output "private_ip_thanos_query3" {
    description = "private_ip of my vm_thanos_query3"
    value = outscale_vm.vm_thanos_query3.private_ip
}